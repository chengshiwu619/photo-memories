...
def run_startup_integrity_check(
    ...
    try:
        ...
        report["checks"].append(
            _make_check(
                ...
                sample_ids=[
                    ...
                    for row in missing_file_refs[:5]
                ],
        ...
        report["checks"].append(
            _make_check(
                ...
                sample_ids=[
                    ...
                    for row in invalid_cover_refs[:5]
                ],
        ...
        report["checks"].append(
            _make_check(
                ...
                len(missing_thumbs),
                sample_ids=[row["file_id"] for row in missing_thumbs[:5]],
        ...
        report["checks"].append(
            _make_check(
                ...
                len(failed_thumbs),
                sample_ids=[row["file_id"] for row in failed_thumbs[:5]],
        ...
        report["checks"].append(
            _make_check(
                ...
                sample_ids=[row["file_id"] for row in broken_thumb_files[:5]],
                sample_paths=[row["thumbnail_path"] for row in broken_thumb_files[:5]],
    ...
    severities = {check["severity"] for check in report["checks"] if check["count"] > 0}
    report["has_errors"] = "error" in severities
    report["has_warnings"] = "warning" in severities
    return report
